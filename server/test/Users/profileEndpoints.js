...
describe('Get/api/users/id/greenpass', () => {
    it('it should fetch an authenticated users greenpass', (done) =>  {
    User.find({}, (err, users) => {
        .get(`/api/users/${user.user_id}/profile/greenpass_available`)
        .end((err, res) => {
            .end((err, res) => {
                res.should.have.status(200);
                res.body.should.be.a('object');
                res.body.should.have.property('greenpass_available').eql(user.greenpass_available);
                done();
            });
        });
    });
...
describe('/Get/api/users/id/greenpass', () => {
	it('it should not fetch an unauthenticated users greenpass', (done) => {
		User.findOne({}, (err, user) => {
			chai.request(server)
				.get(`/api/users/${user.user_id}/profile/greenpass_available`)
				.end((err, res) => {
...
describe('/Get/api/users/id/greenpass', () => {
	it('it should not fetch a non existing users profile', (done) => {
		User.findOne({}, (err, user) => {
			chai.request(server)
				.get(`/api/users/${user.user_id}/profile/greenpass_available`)
				.end((err, res) => {
